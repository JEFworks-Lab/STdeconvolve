...
mob_annot[com==4] <- 'Granule Cell Layer'
mob_annot[com==1] <- 'Mitral Cell Layer'
mob_annot[com==3] <- 'Outer Plexiform Layer'
mob_annot[com==2] <- 'Glomerular Layer'
mob_annot[com==5] <- 'Olfactory Nerve Layer'
...
t <- mob_k15$theta
p <- mobCorpus$mob$pos
txnClusters <- mob_annot

# tcols <- gg_color_hue(ncol(t))
tcols <- rep("black", ncol(t))

group_cc <- c("Glomerular Layer" = transparentCol("red", percent = 0),
              "Granule Cell Layer" = transparentCol("yellow", percent = 0),
              "Mitral Cell Layer" = transparentCol("green", percent = 0),
              "Olfactory Nerve Layer" = transparentCol("blue", percent = 0),
              "Outer Plexiform Layer" = transparentCol("purple", percent = 0))
...
for (i in seq(nT)){
  ...
  plt <- vizAllTopics(theta = m,
                 ...
                 plotTitle =NA,
                 overlay = NA)
  ggsave(filename = paste0("mob_k15", "-topic-", i, ".png"),
         device = "png",
         path = draft_fig_path,
         scale = 1.5,
         width = 5,
...
gplots::heatmap.2(x = corMtx_paired, #corMtx_paired[hc_$order,],
                  Rowv = NA, # to order topics based on pairs and not dendrogram
                  Colv = NA, # as.dendrogram(hc_),
                  density.info = "none",
                  trace = "none",
                  col = correlation_palette,
                  breaks = correlation_breaks,
...
diag_corrs <- diag(corMtx_paired)

hist(diag_corrs, breaks = 10)
mean(diag_corrs)
sd(diag_corrs)
...
stripchart(diag_corrs, vertical = TRUE,
...
3 5 and 12 for granule cell layer
...
group_cc <- c("Glomerular Layer" = transparentCol("black", percent = 0),
              "Granule Cell Layer" = transparentCol("black", percent = 0),
              "Mitral Cell Layer" = transparentCol("black", percent = 0),
              "Olfactory Nerve Layer" = transparentCol("black", percent = 0),
              "Outer Plexiform Layer" = transparentCol("black", percent = 0))
...
other <- 1 - rowSums(m)
m <- cbind(m, other)
colnames(m) <- c(as.character(topics), "other")

cc <- c(
  transparentCol(tcols[topics[1]], percent = 0),
  transparentCol(tcols[topics[2]], percent = 0),
  transparentCol(tcols[topics[3]], percent = 0),
  # transparentCol(tcols[topics[4]], percent = 0),
  # transparentCol(tcols[topics[5]], percent = 0),
  # transparentCol("blue", percent = 50),
  transparentCol("white", percent = 100))
...
plt <- vizAllTopics(theta = m,
               ...
               plotTitle =NA,
               overlay = NA)
ggsave(filename = paste0("mob_k15", "-topics-", "3_5_10-granule", ".png"),
       device = "png",
       path = draft_fig_path,
       scale = 1.5,
       width = 5,
...
C5_gsea_list <- lapply(unique(gene_set$gs_name), function(ct){
  genes <- gene_set[gene_set$gs_name == ct,]$gene_symbol
  genes
})
names(C5_gsea_list) <- unique(gene_set$gs_name)
...
for (i in c(3,5,10)){
  ...
  vals <- sort(mob_k15$beta[i,], decreasing=TRUE)
  print(head(vals))
...
genes <- c("Nrgn", "Penk", "Nrep", "Tubb2b", "Gad1", "Pcp4l1")
df <- t(mob_cpm[genes,])

p <- mobCorpus$mob$pos

df_ <- merge(as.data.frame(p), as.data.frame(df), by=0)
rownames(df_) <- df_$Row.names

for (gene in genes){
  plt <- vizGeneCounts(df_,
              gene,
              groups = NA,
              group_cols = NA,
              size = 5, stroke = 0, a = 1,
              ...
              showLegend = TRUE,
              overlay = NA)
  ggsave(filename = paste0("mob_k15-", gene, ".png"),
         device = "png",
         path = draft_fig_path,
         scale = 1.5,
         width = 5,
...
mes1_markers <- cluster_markers_mob_se_wt_raw[cluster_markers_mob_se_wt_raw$cluster == "Mes1",]
mes1_markers[mes1_markers$gene %in% rownames(mob_cpm),]
...
vlmc_markers[vlmc_markers$gene %in% rownames(mob_cpm),]

```

 shared genes between Mes1 and VLMC
 intersect(vlmc_markers$gene, mes1_markers$gene)

```{r}

# input df for `vizGeneCounts`
marker_genes <- c("Slc13a4", "Col1a1", "Dcn", "Pdgfra", "Lum", "Fam180a")

df <- t(mOB$counts)[rownames(mobCorpus$mob$pos),marker_genes]
df <- merge(mobCorpus$mob$pos, as.data.frame(df), by=0)
rownames(df) <- df$Row.names
...
for (gene in marker_genes){
  vizGeneCounts(df = df,
              ...
              plotTitle = gene,
              showLegend = TRUE)
  ggsave(filename = paste0("VLMC_gexp_", gene, ".pdf"),
         device = "pdf",
         path = draft_fig_path,
         scale = 1.5,
         width = 5,
  ...
  #             plotTitle = gene,
  #             showLegend = TRUE)
  # ggsave(filename = paste0("VLMC_gexp_olf_nerve_", gene, ".pdf"),
  #        device = "pdf",
  #        path = draft_fig_path,
  #        scale = 1.5,
  #        width = 5,
...
# CPM normalize
mob_cpm <- normalizeCounts(counts = t(countsClean), 
                       log=FALSE,
                       verbose=TRUE)
