...
simulate_cell_counts <- function (cell_type_labels, number_spots, max_cells, seed,
                                  merfish_counts = FALSE, merfish_types = FALSE) {
  ...
  if (merfish_types != FALSE) {
    # should be `FN7_2_M22_M26_cell_type_dist`
    merfish_cell_type_probs <- as.vector(merfish_types) # the probabilities for each count
    merfish_cell_type_dist <- as.integer(names(merfish_types)) # the counts themselves
    if (length(cell_type_labels) != length(merfish_cell_type_dist)) {
      stop()
      geterrmessage("If `merfish_types` used, `cell_type_labels` should be equal length")
    }
  }
  
  if (merfish_counts != FALSE) {
    # should be FN7_2_M22_M26_cell_count_dist
    merfish_cell_count_probs <- as.vector(merfish_counts)
    merfish_cell_count_dist <- as.integer(names(merfish_counts))
  }
  ...
  counts <- do.call(rbind, lapply(1:number_spots, function(n) {
    ...
    if ( merfish_types == FALSE) {
      ...
      num_types <- sample(merfish_cell_type_dist, size = 1, prob = merfish_cell_type_probs)
    ...
    if (merfish_counts == FALSE) {
      ...
      tot_cells <- sample(merfish_cell_count_dist, size = 1, prob = merfish_cell_count_probs)
...
### discrete dist for simulations

Collect information to have discrete distributions for which to sample from when making complex simulations:

```{r}

FN7_2_M22_M26_patch_tot_cell_counts <- c()
FN7_2_M22_M26_patch_cell_type_counts <- c()

for (bregma in keys(FN7_2_M22_M26_hash)) {
  
  bregma_patch_counts <- FN7_2_M22_M26_hash[[bregma]][["totalCells"]]
  FN7_2_M22_M26_patch_tot_cell_counts <- append(FN7_2_M22_M26_patch_tot_cell_counts,
                                                bregma_patch_counts)
  
  bregma_patch_cellType_counts <- FN7_2_M22_M26_hash[[bregma]][["cellTypeCount"]]
  FN7_2_M22_M26_patch_cell_type_counts <- append(FN7_2_M22_M26_patch_cell_type_counts,
                                                bregma_patch_cellType_counts)
  
}

# dist for total cells in a spot
FN7_2_M22_M26_cell_count_dist <- table(FN7_2_M22_M26_patch_tot_cell_counts) / sum(table(FN7_2_M22_M26_patch_tot_cell_counts))

merfish_cell_count_probs <- as.vector(FN7_2_M22_M26_cell_count_dist)
merfish_cell_count_dist <- as.double(names(FN7_2_M22_M26_cell_count_dist))


# dist for total cell types in a spot
FN7_2_M22_M26_cell_type_dist <- table(FN7_2_M22_M26_patch_cell_type_counts) / sum(table(FN7_2_M22_M26_patch_cell_type_counts))

merfish_cell_type_probs <- as.vector(FN7_2_M22_M26_cell_type_dist)
merfish_cell_type_dist <- as.double(names(FN7_2_M22_M26_cell_type_dist))

```

Check the relative proportions of cells types for each spot

```{r}

test_cell_type_table <- FN7_2_M22_M26_hash$`-0.24`$cellTypeTable

```

```{r}

# which(FN7_2_M22_M26_hash$`-0.24`$cellTypeCount < 6)
# 
# sort(unique(FN7_2_M22_M26_hash$`-0.24`$cellTypeCount))
# 
# test_cell_type_table / rowSums(test_cell_type_table)

for (i in sort(unique(FN7_2_M22_M26_hash$`-0.24`$cellTypeCount))) {
  
  patches <- test_cell_type_table[which(FN7_2_M22_M26_hash$`-0.24`$cellTypeCount == i),]
  
  label <- paste0("num cell types: ", as.character(i), "; num patches: ", as.character(length(rownames(patches))))
  
  if (length(rownames(patches)) > 1) {
    
    hist(c(patches/rowSums(patches)), breaks = 20, main = label)
    
  } else {
    
    hist(c(patches/sum(patches)), breaks = 20, main = label)
...
## -- dataset: '171216_BD11_1_M22_M26'

```{r}

# select cells that are part of given dataset:
selected_cells <- rownames(features)[features$dataset_name %in% c('171216_BD11_1_M22_M26')]
...
BD11_1_M22_M26_hash <- get_merfish_cell_type_counts(spatial_position_and_class, 100)

```

```{r}

keys(BD11_1_M22_M26_hash)

```

```{r}

for (bregma in keys(BD11_1_M22_M26_hash)) {
  
  hist(BD11_1_M22_M26_hash[[bregma]][["totalCells"]], breaks = 20)
  hist(BD11_1_M22_M26_hash[[bregma]][["cellTypeCount"]], breaks = 10)
...
compare_to_ground_truth <- function(prediction, truth, label, colorscale) {
  ...
  if (dim(corr_mtx_adj)[1] > dim(corr_mtx_adj)[2]) {
    
    print("number ground truth topics > num predicted topics")
    corr_mtx_adj_reorder <- corr_mtx_adj
    
    paired <- colnames(corr_mtx_adj_reorder) # stays the same for now
...
# range between highest and lowest cell type proportions.
# Maybe the good spots have disproportionate distribution of cell types whereas bad spots have cell types at similar proportions. Thus the document is less monochromatic and maybe the model struggles with this
...
# -------------------------------------------------------------
# 5. MERFISH Parameters

Use the MERFISH distribution of number of cell types and total cells in each spot for simulation

## Generate ground truth

```{r}

FN7_2_M22_M26_cell_count_dist

FN7_2_M22_M26_cell_type_dist

levels(pbmc_annot)

# note that there are 11 pmbC cell types, so remove the 11th cell type from the `cell_type_dist` so that they are the same length.

merfish_cell_type_distr <- FN7_2_M22_M26_cell_type_dist[-length(FN7_2_M22_M26_cell_type_dist)]

merfish_cell_count_distr <- FN7_2_M22_M26_cell_count_dist

```

```{r}

merfish_params_pmbcA_sim_spots <- simulate_cell_counts(cell_type_labels = levels(pbmc_annot),
                                            number_spots = 1000,
                                            max_cells = max(as.integer(names(merfish_cell_count_distr))),
                                            seed = 0,
                                            merfish_counts = merfish_cell_count_distr,
                                            merfish_types = merfish_cell_type_distr)

head(merfish_params_pmbcA_sim_spots)

merfish_params_pmbcA_sim_spot_proportions <- merfish_params_pmbcA_sim_spots / rowSums(merfish_params_pmbcA_sim_spots)

head(merfish_params_pmbcA_sim_spot_proportions)
...
merfish_params_pmbcA_sim_spot_genes <- simulate_gene_counts(
                                                  simulated_ct_counts = merfish_params_pmbcA_sim_spots,
                                                  gene_counts = geneCountsClean,
                                                  communities = pbmc_annot,
                                                  spots = 1000,
                                                  seed = 0)

dim(merfish_params_pmbcA_sim_spot_genes)
merfish_params_pmbcA_sim_spot_genes[1:20,1:20]
...
merfish_param_pmbcA_selected <- pbmc_annot[which(pbmc_annot %in% levels(pbmc_annot))]

merfish_param_pmbcA_diffGenes <- getDifferentialGenes(OD_genes, merfish_param_pmbcA_selected)

merfish_param_pmbcA_SigdiffGenes <- lapply(merfish_param_pmbcA_diffGenes, function(x) {
  x <- x[x$p.adj < 0.05,]
  x <- na.omit(x)
  x <- x[x$highest,]
  rownames(x)
...
merfish_param_pmbcA_SigdiffGenes <- as.vector(unlist(merfish_param_pmbcA_SigdiffGenes))
length(merfish_param_pmbcA_SigdiffGenes)
...
# now select the cell types sampled and the significant genes:
merfish_params_pmbCA_ground_truth_topic_words <- ct.gexp.pbmc_annot[levels(pbmc_annot), merfish_param_pmbcA_SigdiffGenes]

merfish_params_pmbCA_ground_truth_topic_words_freq <- merfish_params_pmbCA_ground_truth_topic_words / rowSums(merfish_params_pmbCA_ground_truth_topic_words)

merfish_params_pmbCA_ground_truth_topic_words_freq

```

```{r}

dim(merfish_params_pmbCA_ground_truth_topic_words_freq)

```

```{r}

heatmap(log10(merfish_params_pmbCA_ground_truth_topic_words + 1), scale='none')
...
merfish_params_pmbcA_sim_spot_genes_sig <- t(as.matrix(merfish_params_pmbcA_sim_spot_genes[merfish_param_pmbcA_SigdiffGenes,]))

K=10

# generate model
merfish_param_pmbca_lda <- topicmodels::LDA(slam::as.simple_triplet_matrix(merfish_params_pmbcA_sim_spot_genes_sig),
                                    k=K,
                                    control = list(seed = 0))
...
# matrix of prob of each document associated with topic
merfish_param_pmbca_doc_topics <- tidy(merfish_param_pmbca_lda, matrix = "gamma")
# train.data.doc_topics

# matrix proportions of topics (columns) in each document (rows)
merfish_param_pmbca_topic_proportions <- cast_sparse(merfish_param_pmbca_doc_topics, document, topic, gamma) # row, columns, cells
merfish_param_pmbca_topic_proportions
...
# matrix of prob of each word associated with topic
merfish_param_pmbca_topic_words <- tidy(merfish_param_pmbca_lda, matrix = "beta")

merfish_param_pmbca_topic_words <- as.matrix(cast_sparse(merfish_param_pmbca_topic_words, topic, term, beta)) # row, columns, cells
merfish_param_pmbca_topic_words
...
merfish_params_pmbcA.quick_hdp <- hdp_quick_init(as.matrix(merfish_params_pmbcA_sim_spot_genes_sig))
merfish_params_pmbcA.quick_chain <- hdp_posterior(merfish_params_pmbcA.quick_hdp,
                                                         burnin=100, n=2000, space=1, seed=1234)
...
## check chain convergence
par(mfrow=c(1,3))
plot_lik(merfish_params_pmbcA.quick_chain, bty="L")
plot_numcluster(merfish_params_pmbcA.quick_chain, bty="L")
plot_data_assigned(merfish_params_pmbcA.quick_chain, bty="L")
...
#################### Evaluate performance
## extract results for cell-type proportions
merfish_params_pmbcA.quick_chain <- hdp_extract_components(merfish_params_pmbcA.quick_chain)
merfish_params_pmbcA.hdp_results <- comp_dp_distn(merfish_params_pmbcA.quick_chain)$mean
dim(merfish_params_pmbcA.hdp_results)

merfish_params_pmbcA.hdp_results <- merfish_params_pmbcA.hdp_results[-1,] ## bug it seems

rownames(merfish_params_pmbcA.hdp_results) <- rownames(merfish_params_pmbcA_sim_spot_genes_sig)
colnames(merfish_params_pmbcA.hdp_results) <- paste0(1:ncol(merfish_params_pmbcA.hdp_results))
head(merfish_params_pmbcA.hdp_results)
dim(merfish_params_pmbcA.hdp_results)
...
merfish_params_pmbcA.hdp_topic_words <- comp_categ_distn(merfish_params_pmbcA.quick_chain)$mean
dim(merfish_params_pmbcA.hdp_topic_words)
rownames(merfish_params_pmbcA.hdp_topic_words) <- paste0(1:nrow(merfish_params_pmbcA.hdp_topic_words))
colnames(merfish_params_pmbcA.hdp_topic_words) <- colnames(merfish_params_pmbcA_sim_spot_genes_sig)
head(merfish_params_pmbcA.hdp_topic_words)
...
head(merfish_params_pmbcA_sim_spot_proportions)
merfish_params_pmbCA_ground_truth_topic_words_freq[,1:10]
...
head(merfish_param_pmbca_topic_proportions)

merfish_param_pmbca_topic_words[,1:10]
...
merfish_param_pmbca_lda_doc_topics <- compare_to_ground_truth(
                                            merfish_param_pmbca_topic_proportions,
                                            merfish_params_pmbcA_sim_spot_proportions)
merfish_param_pmbca_lda_doc_topics

heatmap(merfish_param_pmbca_lda_doc_topics$mtx, Rowv=NA, Colv=NA, scale='none',
        col = teal_bordeaux,
        main = "merfish pmbca, lda doc-topic corr_mtx")

heatmap(merfish_param_pmbca_lda_doc_topics$adjmtx, Rowv=NA, Colv=NA, scale='none',
        col = bordeaux,
        main = "merfish pmbca, lda doc-topic corr_mtx_adj")


merfish_param_lda_topic_words <- compare_to_ground_truth(
                                        t(merfish_param_pmbca_topic_words),
                                        t(merfish_params_pmbCA_ground_truth_topic_words_freq))

merfish_param_lda_topic_words

heatmap(merfish_param_lda_topic_words$mtx, Rowv=NA, Colv=NA, scale='none',
        col = teal_bordeaux,
        main = "merfish pmbca, lda topic_words corr_mtx")

heatmap(merfish_param_lda_topic_words$adjmtx, Rowv=NA, Colv=NA, scale='none',
        col = bordeaux,
        main = "merfish pmbca, lda topic-words corr_mtx_adj")

```

If you look at the ground truth labels here that have a strong correlation, they are also pretty distinct if you look at the pmbcA tsne. The others seem to be pretty similar in terms of expression and lo and behold the model also struggles with these.


Because the HDP returned less topics than ground truth, didnt get lsat and so can't compare yet wrt topic proportions in each document. I could do it by hand by reordering the predicted topics that did match.
...
# the paired topics determined from doc-topic proportions
merfish_params_pmbcA_LDA_doc_topic_doccorrs <- documents_topic_correlations(
                                                      merfish_params_pmbcA_sim_spot_proportions,
                                                      merfish_param_pmbca_topic_proportions, 
                                                      as.numeric(merfish_param_pmbca_lda_doc_topics$lsap))

# same doc-topic predictions as above, but the paired topics differs
# the paired topics determined from topic-word correlations
cmerfish_params_pmbcA_LDA_topic_word_doccorrs <- documents_topic_correlations(
                                                            merfish_params_pmbcA_sim_spot_proportions,
                                                            merfish_param_pmbca_topic_proportions,
                                                            as.numeric(merfish_param_lda_topic_words$lsap))

boxplot(merfish_params_pmbcA_LDA_doc_topic_doccorrs)

```

Check to see what the bad spots are. Are they the ones that have a lot of cell types in the spot or do they equal proportions? Also, what spots are the best?


```{r}

# which(merfish_params_pmbcA_LDA_doc_topic_doccorrs < 0.0)
# which(merfish_params_pmbcA_LDA_doc_topic_doccorrs > 0.6)

merfish_params_pmbcA_neg_corr_spots <- merfish_params_pmbcA_sim_spot_proportions[which(merfish_params_pmbcA_LDA_doc_topic_doccorrs < 0.0),]

merfish_params_pmbcA_pos_corr_spots <- merfish_params_pmbcA_sim_spot_proportions[which(merfish_params_pmbcA_LDA_doc_topic_doccorrs > 0.8),]


neg_corr_ranges <- apply(merfish_params_pmbcA_neg_corr_spots[,levels(pbmc_annot)],1,FUN=max) - apply(merfish_params_pmbcA_neg_corr_spots[,levels(pbmc_annot)],1,FUN=min)

pos_corr_ranges <- apply(merfish_params_pmbcA_pos_corr_spots[,levels(pbmc_annot)],1,FUN=max) - apply(merfish_params_pmbcA_pos_corr_spots[,levels(pbmc_annot)],1,FUN=min)

boxplot(neg_corr_ranges, pos_corr_ranges, las=2, cex.names=0.2)

```

Hard to tell but maybe b/c too many cell types to get a good range? Even the most highly correlated spots don't have a significant difference between highest and lowest proportion topics?

Do the best correlated spots have less total topics than the poorly correlated spots?

```{r}

neg_corr_cell_type_counts <- c()

for (i in seq_len(dim(merfish_params_pmbcA_neg_corr_spots)[1])) {
      patch_num_cell_types <- length(which(merfish_params_pmbcA_neg_corr_spots[i,] != 0))
      neg_corr_cell_type_counts <- append(neg_corr_cell_type_counts, patch_num_cell_types)
      
}
neg_corr_cell_type_counts


pos_corr_cell_type_counts <- c()

for (i in seq_len(dim(merfish_params_pmbcA_pos_corr_spots)[1])) {
      patch_num_cell_types <- length(which(merfish_params_pmbcA_pos_corr_spots[i,] != 0))
      pos_corr_cell_type_counts <- append(pos_corr_cell_type_counts, patch_num_cell_types)
      
}
pos_corr_cell_type_counts

boxplot(neg_corr_cell_type_counts, pos_corr_cell_type_counts)
...
merfish_params_pmbcA.hdp_results[1:10,]

merfish_params_pmbcA.hdp_topic_words[,1:10]
...
merfish_param_pmbcA_hdp_doc_topics <- compare_to_ground_truth(merfish_params_pmbcA.hdp_results,
                                                         merfish_params_pmbcA_sim_spot_proportions)

merfish_param_pmbcA_hdp_doc_topics

heatmap(merfish_param_pmbcA_hdp_doc_topics$mtx, Rowv=NA, Colv=NA, scale='none',
        col = teal_bordeaux,
        main = "merfish pmbcA, hdp doc-topic corr_mtx")

heatmap(merfish_param_pmbcA_hdp_doc_topics$adjmtx, Rowv=NA, Colv=NA, scale='none',
        col = bordeaux,
        main = "merfish pmbcA, hdp doc-topic corr_mtx_adj")


merfish_param_pmbcA_hdp_topic_words <- compare_to_ground_truth(t(merfish_params_pmbcA.hdp_topic_words),
                                                      t(merfish_params_pmbCA_ground_truth_topic_words_freq))
merfish_param_pmbcA_hdp_topic_words

heatmap(merfish_param_pmbcA_hdp_topic_words$mtx, Rowv=NA, Colv=NA, scale='none',
        col = teal_bordeaux,
        main = "merfish pmbcA, hdp topic_words corr_mtx")

heatmap(merfish_param_pmbcA_hdp_topic_words$adjmtx, Rowv=NA, Colv=NA, scale='none',
        col = bordeaux,
        main = "merfish pmbcA, hdp topic-words corr_mtx_adj")
...
Play with the vocabulary/topic word dictionary. aka curate the significant genes more

Adjust K with LDA and see if it affects the performance.
...
About 20 cells and 6 cell types per spot. 
Another question: What is the proportion of each cell type in each spot?
Remember that LDA and HDP seem to do ok if there is an imbalance but poor if the topics are represented equally. Probably gets even worse as more topics are also present.

Then try making a new simulated set that reflects the MERFISH results better. Will also help show if more topics evenly dispersed in spots makes things worse.

Then question is how to address this and overcome this?

Could adjusting some of the parameters or choice of K help at all?
