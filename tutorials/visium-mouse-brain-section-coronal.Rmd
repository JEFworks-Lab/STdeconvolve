...
Correlating Louvain clustering to de-convolved cell type is a challenge. Clustering assigns each pixel to a single cluster, whereas de-convolution assigns proportions of each pixel to several cell types. As a first pass solution, suppose that each de-convolved pixel is assigned to the cell type with the largest proportion. This should hold well in homogeneous tissue and break down in heterogeneous tissue or along tissue boundaries.
```{r}
res <- matrix(as.numeric(unfactor(com)), ncol=1)
deconType <- as.numeric(colnames(deconProp)[max.col(deconProp)])
res2 <- data.frame(cbind(names(com), res, deconType))
colnames(res2) <- c("spot", "clusterType", "deconType")

res2 <- aggregate(res2$spot, res2[-1], FUN=length)
res3 <- pivot_wider(res2, names_from = deconType, values_from = x)

# Clustering assignment as row, deconvolved cell type as column
res4 <- matrix(as.numeric(unlist(res3)), ncol = length(res3))
max.col(replace(res4, is.na(res4), -Inf), ties.method = "first")
```
The previous analysis suggest that cluster 1, 2 and 3 all represent cell type 7.
A simple visualization shows that the matching fails to represent most of the area and some other cell types e.g. cell type 20 should also play a prominent role
```{r, fig.height=5, fig.width=5}
# Visualize deconvolved cell type 7
vizTopic(deconProp, pos, 7)

# Visualize deconvolved cell type 18
vizTopic(deconProp, pos, 20)

# Next visualize cluster 1
plotEmbedding(pos, 
              groups=com[com==1], 
              cex=1, xlab=NA, ylab=NA,
              verbose=FALSE)

# Next visualize cluster 2
plotEmbedding(pos, 
              groups=com[com==2], 
              cex=1, xlab=NA, ylab=NA,
              verbose=FALSE)

# Next visualize cluster 3
plotEmbedding(pos, 
              groups=com[com==3], 
              cex=1, xlab=NA, ylab=NA,
              verbose=FALSE)
...
Now, relax the simplification that each pixel is represented by the cell type of highest proportion

Instead, calculate the overlap between each de-convolved cell type and cluster at every pixel, the overlap ranging from 0 - 1. This produces a similarity matrix
```{r}
cluster <- as.numeric(unfactor(com))
mtx <- matrix(0, length(levels(com)), ncol(deconProp))

#TODO: vectorize
for (i in seq(nrow(deconProp))){
  for(j in seq(ncol(deconProp))){
    print(i, j)
    mtx[cluster[i], j] = mtx[cluster[i], j] + deconProp[i,j]
  }
}

max.col(mtx)
```
These clustering seems to be representative of the general region, but not as tight
```{r, fig.height=5, fig.width=5}
# Visualize deconvolved cell type 13
vizTopic(deconProp, pos, 13)
...
Clearly cluster 11 correlates poorly, even though it encompasses swarths of cell type 14. 

Clusters 10 and 11 seems to correlate well with cell type 14.
How do we compare these overlaps?
```{r, fig.height=5, fig.width=5}
# Visualize deconvolved cell type 14
vizTopic(deconProp, pos, 14)


# Next visualize cluster 5
plotEmbedding(pos, 
              groups=com[com==5], 
              cex=1, xlab=NA, ylab=NA,
              verbose=FALSE)

# Next visualize cluster 10
plotEmbedding(pos, 
              groups=com[com==10], 
              cex=1, xlab=NA, ylab=NA,
              verbose=FALSE)

# Next visualize cluster 11
plotEmbedding(pos, 
              groups=com[com==11], 
              cex=1, xlab=NA, ylab=NA,
              verbose=FALSE)
```
A much better solution is converting the graph based cluster vector into a matrix in the same format as the theta matrix. 

```{r, fig.}
comTheta <- model.matrix(~ com + 0)
rownames(comTheta) <- names(com)

mtx <- getCorrMtx(comTheta, deconProp, "t")
rownames(mtx) <- paste0("com_", seq(nrow(mtx)))
colnames(mtx) <- paste0("decon_", seq(ncol(mtx)))

## order the cell-types rows based on best match (highest correlation) with each community 
pairs <- STdeconvolve::lsatPairs(mtx)
m <- mtx[pairs$rowix, pairs$colsix]
correlationPlot(m) +
  ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90))
```
2-3 interesting cases: split topic by graph based clusters
Annotate tissue by annotations e.g. Allan Brain Atlas
  e.g. hybridization

Use beta matrix v.s. Visualise top genes for transcriptional clusters --> recover top genes from PCs
  How does that hash out in split topics?
