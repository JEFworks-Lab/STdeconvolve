...
We focus on the [Visium mouse brain section (coronal) dataset](https://www.10xgenomics.com/resources/datasets/mouse-brain-section-coronal-1-standard-1-1-0). A [summary of the dataset](https://cf.10xgenomics.com/samples/spatial-exp/1.1.0/V1_Adult_Mouse_Brain/V1_Adult_Mouse_Brain_web_summary.html) is provided by 10x Genomics
...
Now, let's see how the deconvolved cell-types compare in terms of clustering the barcodes transcriptionally into 19 clusters.
...
We observe that cell type 11 correlates strongly with transcriptional community 18 and 19. We plot their spatial locations, as well as the image of the tissue slice. Comparing the regions with the [mouse brain reference atlas](https://mouse.brain-map.org/experiment/thumbnails/100048576?image_type=atlas), it appears to overlap with nucleus of the thalamus.

```{r, fig.height=5, fig.width=5}
vizTopic(deconProp, pos, 11, plotTitle = "cell type 11")

com_proxyTheta <- as.matrix(com_proxyTheta)
vizTopic(com_proxyTheta, pos, 18, plotTitle = "communities 18")
vizTopic(com_proxyTheta, pos, 19, plotTitle = "communities 19")

coronal_raster <- brick("~/Github/STdeconvolve/tutorialdata/V1_Adult_Mouse_Brain_image.tif")
plotRGB(coronal_raster)
```
We explore the differentially expressed genes in cell type 11 and transcriptional communities 18 and 19. 
First we identify the upregulated genes (>2). 
110 genes are upregulated in cell type 11, 1141 genes are upregulated in community 18 and 587 genes are upregulated in community 19. 

221 of these genes are common between both communities 18 and 19. 42 of these genes are shared between all three groups.

```{r}
celltype <- c(11)

cell_upreg <- lapply(celltype, function(cluster) {
  diffExp <- deconGexp[celltype,]/colMeans(deconGexp[-celltype,])
  diffExp[is.nan(diffExp)] <- 0
  names(diffExp)[diffExp > 2]
})


communities <- c(18, 19)
avg_gene_exp <- Matrix::rowMeans(counts)

community_upreg <- lapply(communities, function(community) {
  
  spots <- rownames(com_proxyTheta)[com_proxyTheta[,community]==1]
  expression <- counts[,spots] # ~30000 x 102
  diffExp <- Matrix::rowMeans(expression)/avg_gene_exp
  diffExp[is.nan(diffExp)] <- 0
  names(diffExp)[diffExp > 2]
})

lapply(cell_upreg, length)
lapply(community_upreg, length)

upreg <- Reduce(intersect, append(cell_upreg, community_upreg))
upreg <- sort(toupper(upreg))
upreg
```
TODO: compare with Allen Brain Atlas (differentially expressed genes)[https://mouse.brain-map.org/search/show?page_num=0&page_size=100&no_paging=false&domain1=8&domain2=549&domain1_threshold=0,50&domain2_threshold=2,50&image_set=P56coronal&search_type=differential] in the thalamus.
```{r}
...
Let's further explore the 42 upregulated genes. We annotate the genes using GO term. 
```{r}
ah <- AnnotationHub()
orgs <- subset(ah, ah$rdataclass == "OrgDb")
orgdb <- query(orgs, "Homo sapiens")[[1]]
go <- mapIds(orgdb, keys=upreg,
       column="GO", keytype="SYMBOL",
       multiVals="list")
# filter out genes with no GO annotations.
go <- go[!is.na(go)]
go <- unlist(go, use.names = FALSE)

Ontology(go[1])

go2 <- lapply(go, function(go_term){
  if( unname(Ontology(go_term)) == "BP") {
    unname(Term(go_term))
  }
})

go2 <- unlist(plyr::compact(go2), use.names = FALSE)
go_freq <- as.data.frame(table(go2))
go_freq <- go_freq[order(-go_freq$Freq),]
head(go_freq)
```


In other cases, deconvolved cell types form sub-clusters of transcriptional communities. For instance, community 13 correlates strongly with both cell types 3 and 6, and it appears to be split by the deconvolved cell types.

Cell type 6 appears to be the border of ventral posterior medial region. 
Cell type 3...
```{r}
vizTopic(com_proxyTheta, pos, 13, plotTitle = "communities 13")

vizTopic(deconProp, pos, 3, plotTitle = "cell type 3")
vizTopic(deconProp, pos, 6, plotTitle = "cell type 6")
```
