...
mob_se_wt_obj[["nUMI"]] <- mob_meta[which(mob_meta$orig.ident %in% c("WT1", "WT2")),]$nUMI
...
betaCorMtx <- getCorrMtx(m1 = mobWtCellProxyBeta[,colnames(sl_countsClean$betaCt)],
                         m2 = mobWtCellProxyBeta[,colnames(sl_countsClean$betaCt)],
                         type = "b")
...
betaCorMtx <- getCorrMtx(m1 = cortexProxyBeta[,colnames(sl_countsClean_cortexRef$betaCt)],
                         m2 = mobWtCellProxyBeta[,colnames(sl_countsClean$betaCt)],
                         type = "b")
...
betaCorMtx <- getCorrMtx(m1 = mobWtCellProxyBeta,
                           m2 = mob_k38$beta,
                           type = "b")
...
betaCorMtx <- getCorrMtx(m1 = cortexProxyBeta[,colnames(sl_countsClean_cortexRef$betaCt)],
                           m2 = cortexProxyBeta[,colnames(sl_countsClean_cortexRef$betaCt)],
                           type = "b")
...
ENG: endoglin; ENG is selectively expressed on vascular and lymphatic endothelium in tumors. This allows us to target both tumor-associated vasculature and lymphatic vessels to suppress tumor growth and metastasis. ENG is essential for angiogenesis/vascular development and a co-receptor of TGF-β
...
PRSS23: Serine Protease PRSS23 Is Upregulated by Estrogen Receptor α and Associated with Proliferation of Breast Cancer Cells
...
XBP1: XBP1 promotes triple-negative breast cancer by controlling the HIF1α pathway
...
plot(log(m2_filt["Mes1",]), log(m1_filt[4,]))
...
keep_genes <- colnames(mobWtCellProxyBeta)[colnames(mobWtCellProxyBeta) %in% rownames(W_countsClean)]
length(keep_genes)
...
w_vs_mobScRNA_beta <- getCorrMtx(m1 = mobWtCellProxyBeta[,keep_genes],
                                       m2 = t(W_countsClean),
                                       type = "b")
...
# input df for `vizGeneCounts`
df <- t(mob_cpm)[,c("Rps29", "Mt3", "Cst3", "Rpl41", "Tmsb10")]
df <- merge(mobCorpus$mob$pos, as.data.frame(df), by=0)
rownames(df) <- df$Row.names
