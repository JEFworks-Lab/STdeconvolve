...
bcl_k5_02 <- do.call(cbind, lapply(seq(5), function(i){
  bcl_k5$theta[,i] > 0.2
}))
colnames(bcl_k5_02) <- paste(seq(5))
...
desc[grepl('regulation of leukocyte activation', desc$TERM),]
...
for (s in unique(corpus$slice)){
  ...
  slice <- corpus$slice == s
  df_ <- cbind(gexp[slice], p)
  colnames(df_) <- c("gexp", "x", "y")
  ...
  plt <- vizGeneCounts(df_,
              ...
              groups = NA,
              group_cols = NA,
              size = 3, stroke = 0.1, a = 0.7,
              plotTitle = paste("slice", s, "leuk activ GO:0002694"),
              showLegend = TRUE,
              overlay = he)
  ggsave(filename = paste0("bcl_k10-layer-", s, "-", "leuk-activ", ".png"),
         device = "png",
         path = draft_fig_path,
         scale = 1.5,
         width = 5,
...
genes <- c("IGLL5", "PPP1R1B", "JCHAIN", "SPINT2", "LY6E")
df <- t(bcl_CPM[genes,])
...
for (s in unique(corpus$slice)){
  ...
  df_ <- df[slice,]
  df_ <- merge(as.data.frame(p), as.data.frame(df_), by=0)
  ...
  for (gene in genes){
    plt <- vizGeneCounts(df_,
                ...
                group_cols = NA,
                size = 3, stroke = 0.1, a = 0.7,
                plotTitle = paste("slice", s, gene),
                showLegend = TRUE,
                overlay = he)
    ggsave(filename = paste0("bcl_k10-layer-", s, "-", gene, ".png"),
           device = "png",
           path = draft_fig_path,
           scale = 1.5,
           width = 5,
...
for (i in c(6)){
  ...
  # filter for top hits
  gsea.sig <- gsea.results[gsea.results$q.val < 0.1,]
  gsea.sig <- gsea.sig[gsea.sig$sscore > 0,]
  
  gsea.sig <- gsea.sig[order(gsea.sig$p.val),]
  gsea.sig$GOID <- rownames(gsea.sig)
  gsea.sig <- gsea.sig[!grepl("NA", gsea.sig$GOID),]
  
  # print(dim(gsea.sig))
  print(gsea.sig)
  
  plt <- ggplot(data=gsea.sig, aes(x=reorder(GOID, -p.val), y=-log10(p.val))) +
