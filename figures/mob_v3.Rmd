...
shared_genes <- which(rownames(mob_se_wt_raw) %in% colnames(mobLDAsDeconv$`12`$beta))

# mob_se_wt_raw[shared_genes,]

```

```{r}

mobWtCellClusters <- mob_se_wt_obj@meta.data$ClusterName
names(mobWtCellClusters) <- mob_se_wt_obj@meta.data$X

mobWtCellProxyBetaRaw_sharedGenes <- model.matrix(~ 0 + as.factor(mobWtCellClusters))

rownames(mobWtCellProxyBetaRaw_sharedGenes) <- names(mobWtCellClusters)

# fix names
colnames(mobWtCellProxyBetaRaw_sharedGenes) <- unlist(lapply(colnames(mobWtCellProxyBetaRaw_sharedGenes), function(x) {
  unlist(strsplit(x, ")"))[2]
}))

mobWtCellProxyBetaRaw_sharedGenes <- t(as.matrix(mob_se_wt_raw[shared_genes,]) %*% mobWtCellProxyBetaRaw_sharedGenes)

mobWtCellProxyBetaRaw_sharedGenes <- mobWtCellProxyBetaRaw_sharedGenes/rowSums(mobWtCellProxyBetaRaw_sharedGenes)

dim(mobWtCellProxyBetaRaw_sharedGenes)

```

# reference marker gene sets

```{r}

deconGexp <- mobWtCellProxyBetaRaw_sharedGenes*1000
# counts <- as.matrix(simFN7_9ct_135genes$sim)

## for list of markers
mob_gt_gene_set <- list()

for (i in seq(nrow(deconGexp) - 1)){
  celltype <- i
  vals <- sort(deconGexp[celltype,], decreasing = TRUE)
  ...
  p1 <- ggplot(data = dat) +
    geom_col(aes(x = order, y = values, fill = " ")) +
    scale_fill_manual(values = transparentCol("darkgray", percent = 70)) +
    labs(title = paste0(rownames(deconGexp)[celltype], " transcriptional profile"),
         x = "Gene expression rank", y = "Normalized and scaled expression") +
    # coord_cartesian(clip = "off") +
  ...
  mob_gt_gene_set[[ rownames(deconGexp)[celltype] ]] <- markers
...
deconGexp <- mobLDAsDeconv$`12`$beta*1000
gset <- mob_gt_gene_set
...
beta <- mobWtCellProxyBetaRaw[, mob_sig_marker_genes]
## cluster cell-types based on ave gene exp of selected genes
d_ <- as.dist(1-cor(t(beta)))
hc_ <- stats::hclust(d_, method = "ward.D")

rownames(beta)[hc_$order]


corMtx <- getCorrMtx(m1 = mobLDAsDeconv$`12`$beta, #  rows, nrows must be less or equal to ncols for pairing
                     m2 = mobWtCellProxyBetaRaw_sharedGenes, # cols
                     type = "b")

pdf(file = paste0(fig_path, "Supplemental_Figure_S7-D-stdecon_k12_mob_ref_beta_cor-mob_rep8.pdf"),
    width = 8, height = 4)
gplots::heatmap.2(x = corMtx, # rearrange rows based on ct clustering
                  # Rowv = NA,
                  ...
                  breaks = correlation_breaks,
                  key.xlab = "Correlation",
                  xlab = "MOB reference cell-type clusters",
...
## match all 38 ground truth cell-types to the STdeconvolve cell-type they correlate strongest with:
mobRef_vs_lda_beta_all <- apply(corMtx, 2, max)


corMtx <- getCorrMtx(m1 = mobLDAsDeconv$`12`$beta, #  rows, nrows must be less or equal to ncols for pairing
                     m2 = mobWtCellProxyBetaRaw, # cols
                     type = "b")
...
gt_order <- rownames(beta)[hc_$order]
...
d_ <- as.dist(1-cor(t(mobLDAsDeconv$`12`$beta)))
hc_ <- stats::hclust(d_, method = "ward.D")
pred_order <- rownames(mobLDAsDeconv$`12`$beta)[hc_$order]
...
gt_present <- which(gt_order %in% colnames(sl_theta))

corMtx <- getCorrMtx(m1 = lda_k12_theta[,pred_order], #  rows, nrows must be less or equal to ncols for pairing
                     m2 = sl_theta[,gt_order[gt_present]], # cols
                     type = "t")

rownames(corMtx) <- paste0("X", rownames(corMtx))

## sort the deconvolved cell-type rows in order of the highest correlation wrt the txn ordering of the gt cell-types
decon_order_sl <- names(sort(apply(corMtx, 1, which.max)))
corMtx <- corMtx[decon_order_sl,]

pdf(file = paste0(fig_path, "Supplemental_Figure_S7-C-spotlight_vs_stdecon_k12_ct_proportion_cor-mob_rep8.pdf"),
    width = 6, height = 4)
gplots::heatmap.2(x = corMtx, # rearrange rows based on ct clustering
                  Rowv = NA,
                  Colv = NA, # dendrogram also rearranges columns just like the rows and thus pairing is maintained
                  density.info = "none",
                  trace = "none",
                  col = correlation_palette,
                  breaks = correlation_breaks,
                  key.xlab = "Correlation",
                  xlab = "SPOTlight deconvolved cell-types",
                  ylab = "STdevonvolve cell-types",
                  key.title = NA,
                  cexRow = 0.7,
                  cexCol = 0.7,
...
pairings <- rownames(corMtx)[as.vector(apply(corMtx, 2, which.max))]
lda_vs_sl_theta_filt_all <- apply(corMtx, 2, max) # correlations
...
#                   breaks = correlation_breaks,
#                   key.xlab = "Correlation",
#                   xlab = "SPOTlight deconvolved cell-type clusters",
#                   ylab = "STdevonvolve cell-types",
#                   key.title = NA,
#                   cexRow = 0.7,
#                   cexCol = 0.7,
...
# 
# 
# ## match all 26 SL cell-types to the STdeconvolve cell-type they correlate strongest with:
# lda_vs_sl_theta_filt_all <- apply(corMtx, 2, max)
...
gt_present_sl <- which(gt_order %in% colnames(sl_theta))
gt_present_rctd <- which(gt_order %in% colnames(rctd_theta))

corMtx <- getCorrMtx(m1 = rctd_theta[,gt_order[gt_present_rctd]],
                     m2 = sl_theta[,gt_order[gt_present_sl]],
                     type = "t")
...
gplots::heatmap.2(x = corMtx, # rearrange rows based on ct clustering
                  ...
                  key.xlab = "Correlation",
                  xlab = "SPOTlight deconvolved cell-types",
                  ylab = "RCTD deconvolved cell-types",
                  key.title = NA,
                  cexRow = 0.7,
                  cexCol = 0.7,
                  # lhei = c(1,3),
...
pairings <- rownames(corMtx)[as.vector(apply(corMtx, 2, which.max))]
rctd_vs_sl_theta_filt_all <- apply(corMtx, 2, max) # correlations
...
# 
# ## match all 26 SL cell-types to the RCTD cell-type they correlate strongest with:
# rctd_vs_sl_theta_filt_all <- apply(corMtx, 2, max)
...
gt_present <- which(gt_order %in% colnames(rctd_theta))

corMtx <- getCorrMtx(m1 = lda_k12_theta[,pred_order], #  rows, nrows must be less or equal to ncols for pairing
                     m2 = rctd_theta[,gt_order[gt_present]], # cols
                     type = "t")

rownames(corMtx) <- paste0("X", rownames(corMtx))

## sort the deconvolved cell-type rows in order of the highest correlation wrt the txn ordering of the gt cell-types
decon_order_rctd <- names(sort(apply(corMtx, 1, which.max)))
corMtx <- corMtx[decon_order_sl,]
...
gplots::heatmap.2(x = corMtx, # rearrange rows based on ct clustering
                  ...
                  breaks = correlation_breaks,
                  key.xlab = "Correlation",
                  xlab = "RCTD deconvolved cell-types",
                  ylab = "STdevonvolve cell-types",
                  key.title = NA,
                  cexRow = 0.7,
                  cexCol = 0.7,
...
pairings <- rownames(corMtx)[as.vector(apply(corMtx, 2, which.max))]
rctd_vs_lda_theta_filt_all <- apply(corMtx, 2, max) # correlations
...
#                   key.xlab = "Correlation",
#                   xlab = "RCTD deconvolved cell-types",
#                   ylab = "STdevonvolve deconvolved cell-types",
#                   key.title = NA,
#                   cexRow = 0.7,
#                   cexCol = 0.7,
#                   # lhei = c(1,3),
...
# 
# ## match all 22 RCTD cell-types to the STdeconvolve cell-type they correlate strongest with:
# rctd_vs_lda_theta_filt_all <- apply(corMtx, 2, max)
...
cor_mob_summary <- list("STdeconvolve\nvs\nMOB ref gene expression" = mobRef_vs_lda_beta_all,
                        "STdeconvolve\nvs\nRCTD" = rctd_vs_lda_theta_filt_all,
                        "RCTD\nvs\nSL" = rctd_vs_sl_theta_filt_all,
                        "STdeconvolve \ns\nSL" = lda_vs_sl_theta_filt_all
  )

## edit group order on plot
dat <- reshape2::melt(plyr::ldply(cor_mob_summary, rbind))
dat$group <- factor(dat$.id, levels = c("STdeconvolve\nvs\nMOB ref gene expression",
                                        "STdeconvolve\nvs\nRCTD",
                                        "RCTD\nvs\nSL",
                                        "STdeconvolve \ns\nSL"))

ggplot(data = dat,
       aes(x = reorder(group, group), y = value, fill = group)) +
...
#        aes(x=.id, y=value, fill=.id)) +
  ...
  labs(title = "Correlations between deconvolved cell-types",
  ...
  theme(axis.text.x = element_text(angle = 0, size = 12),
        ...
        plot.title = element_text(size=15, face=NULL),
        legend.position = "none")
ggplot2::ggsave(filename = paste0("Supplemental_Figure_S7-E-compare_method_ct_proportions_boxplots.pdf"),
         device = "pdf",
         path = fig_path,
         scale = 1.5,
         width = 5,
