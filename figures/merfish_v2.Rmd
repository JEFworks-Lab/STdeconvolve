...
Tables of number of celltypes in each spot for entire FN7 animal
...
gplots::heatmap.2(x = corMtx,
                  ...
                  breaks = correlation_breaks,
                  key.xlab = "Correlation",
                  xlab = "Actual cell-types",
                  ylab = "Predicted cell-types",
                  key.title = NA,
                  cexRow = 1,
                  cexCol = 0.9,
                  # lhei = c(1,3),
...
ggplot(data = reshape2::melt(corMtx[rev(pairs$rowix), pairs$colsix])) +
  ...
  ggtitle("Cell-type pixel proportion correlation") +
...
corMtx <- getCorrMtx(m1 = FN7_LDAs_20um_k8$beta,
...
for(ix in names(simBregmasFN7)){
  ...
  p1 <- vizAllTopics(theta = m,
                     ...
                     plotTitle = ix)
  print(p1)
  ggsave(filename = paste0("0_8cts_STdev_bregma_", ix, ".png"),
         device = "png",
         path = fig_path,
         scale = 1.5,
         width = 5,
...
corMtx <- getCorrMtx(m1 = lda_k8_theta,
...
# pair up best matching cell class to a predicted topic
pairs <- lsatPairs(corMtx)
gplots::heatmap.2(x = corMtx[rev(pairs$rowix), pairs$colsix],
                  Rowv = NA, # to order topics based on pairs and not dendrogram
                  Colv = NA,
                  density.info = "none",
                  trace = "none",
...
corMtx <- getCorrMtx(m1 = lda_k75_theta,
...
ggplot(data = reshape2::melt(corMtx[rev(pairs$rowix), pairs$colsix])) +
  ...
  xlab("Ground truth cell-type") +
  ylab("Predicted cell-type") +
  ggtitle("Cell-type gene expression correlation") +
  theme(axis.text.x = element_text(angle=-90),
        text = element_text(size=6),
        panel.grid = element_blank(),
        axis.line = element_blank(),
...
ggplot(data = reshape2::melt(raw_75cts),
  ...
  geom_boxplot(outlier.shape=NA) +
  geom_point(position = position_jitterdodge(), alpha = 0.3, size = 0.1) + 
  labs(title = "MERFISH cell-subtype predictions in FN7 spots",
       x = "Method", y = "RMSE") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
# ggsave(filename = "Fig_2-RMSEraw-mjr-cts.pdf",
...
rmses <- as.vector(l_eachCt)
# breg <- rep(ix, length(cts))
tool <- rep("STdeconvolve all cts", length(cts))
rmse_type <- rep("Raw", length(cts))
# total ct proportions
tot_p <- colSums(FN7_neuro_cellTypeTable)/sum(colSums(FN7_neuro_cellTypeTable))
proportion <- as.vector(tot_p[cts])
...
ggplot(data = k8_raw_ct_rmse_summary_nrmse, aes(x = Method, y = RMSE, color = Cell.type)) +
...
ggplot(data = k75_raw_ct_rmse_summary_nrmse, aes(x = Method, y = RMSE, color = Cell.type)) +
  geom_point(position=position_jitterdodge(0.1), cex=1) +
  # geom_jitter(aes(x = variable, y = value, color = group), position=position_jitter(0.1), cex=0.1) +
  # geom_jitter(data = reshape2::melt(truth_theta), aes(x = Var2, y = value), position=position_jitter(0.1), cex=0.1) +
  # scale_y_continuous(breaks = seq(0,max(predict_theta),1)) +
...
ggplot(data = k8_raw_ct_rmse_summary_cosine, aes(x = Method, y = RMSE, color = Cell.type)) +
  geom_point(position=position_jitterdodge(0.1), cex=1) +
  # geom_jitter(aes(x = variable, y = value, color = group), position=position_jitter(0.1), cex=0.1) +
  # geom_jitter(data = reshape2::melt(truth_theta), aes(x = Var2, y = value), position=position_jitter(0.1), cex=0.1) +
  # scale_y_continuous(breaks = seq(0,max(predict_theta),1)) +
...
ggplot(data = k75_raw_ct_rmse_summary_cosine, aes(x = Method, y = RMSE, color = Cell.type)) +
  geom_point(position=position_jitterdodge(0.1), cex=1) +
  # geom_jitter(aes(x = variable, y = value, color = group), position=position_jitter(0.1), cex=0.1) +
  # geom_jitter(data = reshape2::melt(truth_theta), aes(x = Var2, y = value), position=position_jitter(0.1), cex=0.1) +
  # scale_y_continuous(breaks = seq(0,max(predict_theta),1)) +
...
ggplot(data = k8_raw_ct_rmse_summary, aes(x = Proportion, y = RMSE, color = Cell.type)) +
  ...
  facet_wrap(~Method) +
  theme_classic() +
  labs(title = "MERFISH 8 cell-type predictions by method", y = "RMSE")

ggplot(data = k8_scaled_ct_rmse_summary, aes(x = Proportion, y = RMSE, color = Cell.type)) +
  geom_point() +
  facet_wrap(~Method) +
  theme_classic() +
  labs(title = "MERFISH 8 cell-type predictions by method", y = "Scaled RMSE")

ggplot(data = k8_raw_ct_rmse_summary_nrmse, aes(x = Proportion, y = RMSE, color = Cell.type)) +
  geom_point() +
  facet_wrap(~Method) +
  theme_classic() +
  labs(title = "MERFISH 8 cell-type predictions by method", y = "NRMSE")

ggplot(data = k8_raw_ct_rmse_summary_cosine, aes(x = Proportion, y = RMSE, color = Cell.type)) +
  geom_point() +
  facet_wrap(~Method) +
  theme_classic() +
  labs(title = "MERFISH 8 cell-type predictions by method", y = "Cosine Similarity")
...
# perplexity cell type proportion
...
fitLDA2(counts = as.matrix(simFN7$sim),
        seed = 0,
        Ks = seq(2,20, by = 2),
        testSize = 0.2,
        ncores = 7,
