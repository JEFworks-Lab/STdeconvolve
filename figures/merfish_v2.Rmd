...
lda_k9_135genes_theta[lda_k9_135genes_theta < 0.05] <- 0
...
## final figure illustrator colors
...
plts1_grid
ggplot2::ggsave(filename = paste0("Supplementary_Figure_S1-C-bregma_All_stdecon_9cts_135genes.png"),
                  plot = plts1_grid,
                  device = "png",
                  path = fig_path,
                  scale = 1,
...
plts2 <- lapply(names(simBregmasFN7), function(ix){
  ...
  p2 <- ggplot() +
    geom_point(data = breg,
               aes(x = Centroid_X, y = Centroid_Y, color = Cell_class), size=0.001, shape = 21) +
...
plts2_grid
ggplot2::ggsave(filename = paste0("Supplementary_Figure_S1-A-bregma_All_gt_grid_9cts_135genes.png"),
                plot = plts2_grid,
                device = "png",
                path = fig_path,
                scale = 1,
...
plts3 <- lapply(names(simBregmasFN7), function(ix){
  ...
  p3 <- vizAllTopics(theta = m,
               pos = pos,
               topicOrder=seq(ncol(m)),
               topicCols=as.vector(classCols[colnames(m)[colSums(m) > 0]]), # some bregmas may not have all of the cell-types
               # topicCols=as.vector(classCols[colnames(m)]),
               groups = rep("0", dim(m)[1]),
               group_cols = c("0" = "black"),
...
plts3_grid
ggplot2::ggsave(filename = paste0("Supplementary_Figure_S1-B-bregma_All_simulated_9cts_135genes.png"),
                plot = plts3_grid,
                device = "png",
                path = fig_path,
                scale = 1,
...
plts4 <- lapply(names(simBregmasFN7), function(ix){
  ...
  p4 <- vizAllTopics(theta = m,
                     ...
                     topicCols=as.vector(classColsSTdev),
                     groups = rep("0", dim(m)[1]),
                     group_cols = c("0" = "black"),
                     r = 40,
                     lwd = 0.01,
...
transcriptional[8] <- corMtx_txn_1["Excitatory", "Cell-type 2"]

proportional <- diag(corMtx_prop_4)
proportional[7] <- corMtx_prop_4["Inhibitory", "Cell-type 7"]
proportional[8] <- corMtx_prop_4["Excitatory", "Cell-type 2"]
...
lda_k76_135genes_theta[lda_k76_135genes_theta < 0.05] <- 0
...
deconGexp <- gtCtNeuronalGenesFN7*1000
# counts <- as.matrix(simFN7_9ct_135genes$sim)

## for list of markers
gt_gene_set_neuro <- list()

for (i in seq(nrow(gtCtNeuronalGenesFN7))){
  celltype <- i
  vals <- sort(deconGexp[celltype,], decreasing = TRUE)
  ...
  ## with log2FC > 1 (so FC > 2 over the mean exp of the other cell-types)
  markers <- names(log2fc[log2fc > 1])
  gt_gene_set_neuro[[ rownames(deconGexp)[celltype] ]] <- markers
...
## the properly ordered txn correlation matrix
corMtx_txn_1 <- corMtx_txn[pairs$rowix, pairs$colsix]
colnames(corMtx_txn_1) <- paste0("X", colnames(corMtx_txn_1))
...
## plot the txn correlations
dat <- reshape2::melt(corMtx_txn_1[gt_order, pred_order]) #  corMtx_txn_cs
ggplot2::ggplot(data = dat) +
  ggplot2::geom_tile(aes(x = Var1, y = Var2, fill=value)) +
  ggplot2::scale_fill_gradientn(colors = correlation_palette, breaks = correlation_breaks, limits = c(-1,1),
                                guide = guide_colorbar(title = "correlation", ticks = FALSE, label = FALSE)) +
...
corMtx_prop_1 <- getCorrMtx(m1 = as.matrix(gt$gtSpotTopics)[,ct_txn_order],
                          m2 = filt_theta[,pairs$colsix],
                          type = "t")
colnames(corMtx_prop_1) <- paste0("X", colnames(corMtx_prop_1))
...
# plot the proportion correlations, cell-types ordered based on transcriptional pairing
dat <- reshape2::melt(corMtx_prop_1[gt_order, pred_order]) #  corMtx_prop_cs
ggplot2::ggplot(data = dat) +
  ggplot2::geom_tile(aes(x = Var1, y = Var2, fill=value)) +
  ggplot2::scale_fill_gradientn(colors = correlation_palette, breaks = correlation_breaks, limits = c(-1,1),
                                guide = guide_colorbar(title = "correlation", ticks = FALSE, label = FALSE)) +
...
## the properly ordered proportional correlation matrix
corMtx_prop_4 <- corMtx_prop_4[pairs$rowix, pairs$colsix]
colnames(corMtx_prop_4) <- paste0("X", colnames(corMtx_prop_4))
...
transcriptional <- diag(corMtx_txn_1)
transcriptional[7] <- corMtx_txn_1["Inhibitory", "X7"]
transcriptional[8] <- corMtx_txn_1["Inhibitory", "X2"]

proportional <- diag(corMtx_prop_4)
proportional[7] <- corMtx_prop_4["Inhibitory", "X7"]
proportional[8] <- corMtx_prop_4["Inhibitory", "X2"]
...
dat <- simFN7_9ct_135genes
corpus <- as.matrix(dat$sim)
Ks <- c(9)
...
set.seed(numReps) # for sampling
seed <- 1 # for LDA
## for each replicate, get a sample of pixel of size d
s <- sample(nrow(corpus), numDocs)
sampleCorpus <- corpus[s,]
gtCorpus <- dat$gtSpotTopics[s,]
## drop cell-types that are 0 for all sampled pixels
...
# for(d in numDocs){
  ...
  for(r in numReps){
    ...
    for(k in Ks){
      ...
      ## pair instead using gsea
      deconGexp <- res$beta*1000
      gset <- gt_gene_set
      assignments <- c()
      for (i in seq(nrow(deconGexp))){
        celltype <- i
        ...
        gsea.sig <- gsea.sig %>%
          arrange(q.val, -sscore, -edge)
        assignments <- c(assignments, rownames(gsea.sig)[1])
      }
      names(assignments) <- colnames(res$theta)
      assignments <- as.factor(assignments)
      
      print(assignments)
      
      pred_theta <- do.call(cbind, lapply(levels(assignments), function(ct){
        matches <- which(assignments == ct)
        if (length(matches) > 1){
          combnThetas <- rowSums(res$theta[,matches])
        } else {
          combnThetas <- res$theta[,matches]
        }
        combnThetas
      }))
      colnames(pred_theta) <- levels(assignments)
      
      ## make sure pred_theta has same columns in same order as truth_theta
      predict_theta <- do.call(cbind, lapply(colnames(gtCorpus), function(ct){
        if (ct %in% colnames(pred_theta)){
          vals <- pred_theta[,ct]
        } else {
          vals <- rep(0, dim(gtCorpus)[1])
        }
        vals
      })) 
      colnames(predict_theta) <- colnames(gtCorpus)
      truth_theta <- gtCorpus
      
      print(res$theta[1:3,])
      print(predict_theta[1:3,])
      print(truth_theta[1:3,])
...
## 2 and 8 as Excitatory
## 7 and 4 as inhibitory
## pericytes and microglias not detected

predict_theta <- breg_theta[,theta_pairs$colsix]
predict_theta <- cbind(predict_theta, rowSums(predict_theta[,c("4","7")]))
predict_theta <- cbind(predict_theta, rowSums(predict_theta[,c("2", "8")]))

## Cell-type 1   Cell-type 8   Cell-type 4   Cell-type 9   Cell-type 3   Cell-type 6 
## "OD Immature"  "Excitatory"  "Inhibitory"   "Astrocyte"   "Ependymal" "Endothelial" 
##   Cell-type 7   Cell-type 2   Cell-type 5 
## "Inhibitory"  "Excitatory"   "OD Mature"

## column names of predict_theta in same order as above.
## new columns added for the combined 4+7 inhibitroy and 2+8 excitatory
## set 7 and 2 to Pericytes and Microglia because these columns will be set to all 0's anyways below
colnames(predict_theta) <- c("OD Immature", "Excitatory_drop",  "Inhibitory_drop",  "Astrocyte", "Ependymal", "Endothelial", "Pericytes",   "Microglia", "OD Mature", "Inhibitory", "Excitatory")
...
## pair instead using gsea
deconGexp <- FN7_K76_135genes$beta*1000
gset <- gt_gene_set_neuro
assignments <- c()
for (i in seq(nrow(deconGexp))){
  celltype <- i
  vals <- sort(deconGexp[celltype,], decreasing = TRUE)
...
names(assignments) <- colnames(breg_theta)
assignments <- as.factor(assignments)
...
pred_theta <- do.call(cbind, lapply(levels(assignments), function(ct){
  matches <- which(assignments == ct)
  if (length(matches) > 1){
    combnThetas <- rowSums(breg_theta[,matches])
  } else {
    combnThetas <- breg_theta[,matches]
  }
  combnThetas
}))
colnames(pred_theta) <- levels(assignments)
...
## make sure pred_theta has same columns in same order as truth_theta
predict_theta <- do.call(cbind, lapply(colnames(breg_gt_theta), function(ct){
  if (ct %in% colnames(pred_theta)){
    vals <- pred_theta[,ct]
  } else {
    vals <- rep(0, dim(breg_gt_theta)[1])
  }
  vals
})) 
colnames(predict_theta) <- colnames(breg_gt_theta)
truth_theta <- breg_gt_theta
...
dat$variance <- dat$Spot.proportion.std^2

dat$selectedLabels <- ""
ix_label <- which(dat$Cell.type %in% c("Pericytes", "Ependymal", "Microglia"))
...
ggplot(data = dat, aes(x = variance, y = RMSE, color = Cell.type)) +
  geom_point(size=1) +
  ggrepel::geom_text_repel(data = dat, mapping = aes(x = variance, y = RMSE, label = selectedLabels), size = 3, 
                    min.segment.length = 1.5, seed = 42, box.padding = 0.1, max.overlaps = Inf, point.padding = 0.01,
                    # fill = "white", xlim = c(-Inf, Inf), ylim = c(-Inf, Inf), 
                    nudge_x = 0.03, direction = "both", hjust = "left", nudge_y = 2
                    # segment.curvature = -0.1, segment.ncp = 3, segment.angle = 20)
