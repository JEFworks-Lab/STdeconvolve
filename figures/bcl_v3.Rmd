...
bcl_colors <- bcl_com
levels(bcl_colors)[levels(bcl_colors) == "1"] <- "green" # non-malignant
levels(bcl_colors)[levels(bcl_colors) == "2"] <- "blue" # ductal carcinoma in situ
levels(bcl_colors)[levels(bcl_colors) == "3"] <- "red" # Invasive carcinoma 
...
names(bclTxnClustCols) <- c("NonInvasive", "Ductal", "Invasive")
colnames(bclProxyTheta) <- c("NonInvasive", "Ductal", "Invasive")
```

```{r}
x <- bclCorpus$txn
levels(x)[levels(x)=="1"] <- "NonInvasive"
levels(x)[levels(x)=="2"] <- "Ductal"
levels(x)[levels(x)=="3"] <- "Invasive"
bclCorpus$txn <- x
```
...
plts_grid
ggplot2::ggsave(filename = paste0("4_txn_clusters.png"),
         device = "png",
         plot = plts_grid,
         path = fig_path,
         scale = 1.5,
...
pdf(file = paste0(fig_path, "4_bcl_k15_vs_txn_corr_heatmap.pdf"), width = 8, height = 4)
gplots::heatmap.2(x = corMtx,
                  density.info = "none",
                  trace = "none",
                  col = correlation_palette,
...
plts_grid
ggplot2::ggsave(filename = paste0("4_k15_pixels_and_txn_clusters.png"),
         device = "png",
         plot = plts_grid,
         path = fig_path,
         scale = 1.5,
...
plts_grid
ggplot2::ggsave(filename = paste0("4_k15_pixels.png"),
         device = "png",
         plot = plts_grid,
         path = fig_path,
         scale = 1.5,
...
for(celltype in seq(15)){
  ...
  # plts_grid
  ggplot2::ggsave(filename = paste0("4_k15_ct", celltype, "_with_txnClusters.png"), # 4_k15_ct15_overlays.png
           device = "png",
           plot = plts_grid,
           path = fig_path,
           scale = 1.5,
...
ggplot2::ggsave(filename = paste0("4_k15_ct15_txnProfile_barplot.png"),
         device = "png",
         path = fig_path,
         scale = 1.5,
         width = 5,
...
ggplot2::ggsave(filename = paste0("4_k15_ct15_txnProfile_log2FC_barplot.png"),
         device = "png",
         path = fig_path,
         scale = 1.5,
         width = 5,
...
ggplot2::ggsave(filename = paste0("4_k15_ct15_top_go_terms_barplot.png"),
         device = "png",
         path = fig_path,
         scale = 1.5,
         width = 8,
...
for (s in unique(bclCorpus$slice)){
  ...
  print(plts_grid)
  ggplot2::ggsave(filename = paste0("4_k15_section_", s, "_immune_go_terms_gexp_counts.png"),
           device = "png",
           plot = plts_grid,
           path = fig_path,
           scale = 1.5,
...
# plts_grid
ggplot2::ggsave(filename = paste0("4_k15_ct15_overlays_with_txnClusters.png"), # 4_k15_ct15_overlays.png
         device = "png",
         plot = plts_grid,
         path = fig_path,
         scale = 1.5,
...
markers <- c("KRT17", "PRSS23", "GBP5", "CD74", "CXCL10")
for (marker in markers){
  ...
  print(plts_grid1)
    ggplot2::ggsave(filename = paste0("4_k15", "_", marker, "_all_sections_gexp_counts.png"),
             device = "png",
             plot = plts_grid1,
             path = fig_path,
             scale = 1.5,
