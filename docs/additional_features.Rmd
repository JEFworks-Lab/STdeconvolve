...
To demonstrate, let's use the mouse olfactory bulb (mOB) dataset provided with `STdeconvolve`:
...
Let's check out the different objects stored in the output list `mobCorpus1`:
...
genes. As an example, we will take the cleaned count matrix of the entire transcriptome of the mOB dataset and filter for just the overdispersed genes. We can use this vector of genes to select for only these genes in the corpus. (Note that this is for demonstration purposes. Overdispersed gene can be obtained in `preprocess()` by setting the parameter `ODgenes = TRUE`. In reality, one may already have a list of marker genes in mind and can select for these genes specifically).
