...
vignette: >
  %\VignetteIndexEntry{Process MPOA Data}
...
After, the gene counts and metadata tables can be used as input in functions included in `STdeconvolve` functions in `R/misc.R` (on the `devel` branch) to generate simulated ST pixels.
