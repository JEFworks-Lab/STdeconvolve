...
## Question: does using the ideal set of marker genes improve correspondence
## between deconvolved topics and original single-cell RNA-seq data
...
dg <- MERINGUE::getDifferentialGenes(cd.wt, annot.wt)
...
countsFilt <- counts[intersect(markers, rownames(counts)),]

## remove genes that are present in more than X% of spots
vi <- rowSums(countsFilt > 0) > ncol(countsFilt)*1
table(vi)
countsFiltnoUnifGenes <- countsFilt[!vi,]
dim(countsFiltnoUnifGenes)

## can this set of genes still separate single cells?
mat <- MERINGUE::normalizeCounts(as.matrix(cd.wt[rownames(countsFiltnoUnifGenes),]),
                                 log=TRUE)
emb <- Rtsne::Rtsne(as.matrix(t(mat)))$Y
rownames(emb) <- colnames(mat)
head(emb)
par(mfrow=c(1,1))
plotEmbedding(emb, groups=annot.wt, mark.clusters = TRUE)
...
################# How do these optimal markers compare with ods genes
optim.markers <- rownames(countsFiltnoUnifGenes)
ods.markers <- MERINGUE::getOverdispersedGenes(counts, plot = TRUE, details = TRUE, alpha = 0.1)$ods

length(intersect(optim.markers, ods.markers))
length(setdiff(optim.markers, ods.markers))

## if we only use intersection
#test.genes <- intersect(optim.markers, ods.markers)
test.genes <- setdiff(optim.markers, ods.markers)

## can this set of genes still separate single cells?
mat <- MERINGUE::normalizeCounts(as.matrix(cd.wt[test.genes,]),
                                 log=TRUE)
mat <- as.matrix(mat)
mat[1:5,1:5]
mat[is.na(mat)] <- 0
...
## topic modeling
mob_corpus <- as.matrix(countsFiltnoUnifGenes[test.genes,])
mob_corpus_slamMtx <- slam::as.simple_triplet_matrix(mob_corpus)
...
## scale reference to million then log transform
good.genes <- intersect(colnames(mob_lda.beta), rownames(counts.summary.mm))
length(good.genes) ## should be same as testgenes
mat.summary <- MERINGUE::normalizeCounts(counts.summary.mm[good.genes,], log=TRUE)
## scale to million and log transform
beta.cpm <- MERINGUE::normalizeCounts(t(mob_lda.beta[,good.genes]), log=TRUE)
beta.cpm <- as.matrix(beta.cpm)
